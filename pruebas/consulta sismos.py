...
class VentanaPrincipal(QMainWindow):
    ...
    def _actualizar_encabezado_salida(self):
        """
        Inserta/actualiza las dos primeras líneas de la salida:
        ...
        # Remover encabezados previos si existen (líneas que inician con "Origen:" y "Destino:")
        # para evitar duplicados y mantener cualquier texto posterior.
        resto = []
        if lineas:
            i = 0
            # Si las dos primeras líneas tienen el formato de encabezado, las saltamos
            if len(lineas) >= 2 and lineas[0].startswith("Origen:") and lineas[1].startswith("Destino:"):
                resto = lineas[2:]
            else:
                # Si no están en cabecera, conservamos todo como 'resto'
                resto = lineas
        ...
        encabezado = [f"Origen: {origen_str}", f"Destino: {destino_str}", ""]
